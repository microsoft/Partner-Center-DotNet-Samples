...
namespace Microsoft.Store.PartnerCenter.Samples.Agreements
{
    ...
    public class GetAllCustomersAgreements : BasePartnerScenario
    {
        ...
        protected override void RunScenario()
        {
            ...
            string csvFilePath = ObtainCustomersAgreementCsvFileName();
            File.WriteAllText(csvFilePath, $"TenantId,Domain,Date,First Name,Last Name,Phone,Email {Environment.NewLine}");

            // query the customers, get the first page if a page size was set, otherwise get all customers
            SeekBasedResourceCollection<Models.Customers.Customer> customersPage = (customerPageSize <= 0) ? partnerOperations.Customers.Get() : partnerOperations.Customers.Query(QueryFactory.Instance.BuildIndexedQuery(this.customerPageSize));
            ...
            while (customersEnumerator.HasValue)
            {
                foreach (Models.Customers.Customer customer in customersEnumerator.Current.Items)
                {
                    try
                    {
                        ...
                        Context.ConsoleHelper.WriteObject($"#{++count} Tenant: {customer?.CompanyProfile?.TenantId ?? customer?.Id}, Domain: {customer?.CompanyProfile?.Domain ?? "Domain not avaialble." }", "Customer");
                        ...
                        {
                            Context.ConsoleHelper.WriteObject(noAgreements, "Agreement", 1);
                            File.AppendAllText(csvFilePath, $"{customer?.CompanyProfile?.TenantId ?? customer?.Id}, {customer?.CompanyProfile?.Domain ?? "Domain not avaialble." }{Environment.NewLine}");
                        }
                        else
                        {
                            foreach (Agreement customerAgreement in customerAgreements.Items)
                            {
                                Context.ConsoleHelper.WriteObject($"Date: {customerAgreement.DateAgreed}, First Name: {customerAgreement.PrimaryContact.FirstName}, Last Name: {customerAgreement.PrimaryContact.LastName}, Phone: {customerAgreement.PrimaryContact.PhoneNumber}, Email: {customerAgreement.PrimaryContact.Email}", "Agreement", 1);
                                File.AppendAllText(csvFilePath, $"{customer?.CompanyProfile?.TenantId ?? customer?.Id},{customer?.CompanyProfile?.Domain ?? "Domain not avaialble." },{customerAgreement.DateAgreed},{customerAgreement.PrimaryContact.FirstName},{customerAgreement.PrimaryContact.LastName},{customerAgreement.PrimaryContact.PhoneNumber},{customerAgreement.PrimaryContact.Email}{Environment.NewLine}");
                            }
                        }
                    }
            ...
            Context.ConsoleHelper.WriteObject($"Total Customers: {count} processed in {DateTime.UtcNow - startTime}.");
        }
    }
}